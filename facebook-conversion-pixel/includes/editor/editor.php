		...
		$html .= '<table id="fca_pc_main_table" class="fca_pc_setting_table"  >';
			$html .= "<tr>";
				...
				if ( FCA_PC_PLUGIN_PACKAGE !== 'Lite' ){
					$html .= '<button type="button" id="fca_pc_new_pixel_id" class="button button-secondary" title="' . __('Add Pixel ID', 'facebook-conversion-pixel' ) . '">';
					$html .= '<span class="dashicons dashicons-plus" style="vertical-align: middle;"></span>Add New';
					$html .= '</button>';
				}
				$html .= '</br><a class="fca_pc_hint" href="https://fatcatapps.com/facebook-pixel-wordpress-plugin/#pixel-id" target="_blank">' . __( 'What is my Facebook Pixel ID?', 'facebook-conversion-pixel' ) . '</a>';
				$html .= '</td>';
			$html .= "</tr>";
		...
		<?php } else {
			...
			<table class='fca_pc_integrations_table'>
				...
				<tr class='fca-pc-woo-feed-settings'>
					<th><?php _e('Include Variations', 'facebook-conversion-pixel') ?></th>
						<td><?php echo fca_pc_input( 'woo_variations', '', $woo_include_variations, 'checkbox' ) ?>
					<span class='fca_pc_hint'><?php _e("Having a lot of product variations can cause load issues with your feed, disable to exclude variations from the feed.", 'facebook-conversion-pixel') ?></span></td>
				</tr>
