...
if ( !$upgraded && $has_legacy_save ) {
	...
	define( 'FCA_PC_PLUGIN_VER', '2.5.6' );
	...
	function fca_pc_admin_notice() {
		...
		if ( FCA_PC_PLUGIN_PACKAGE === 'Lite' ){
			...
			if ( $show_review_option && $show_review_option !== 'not-set' ){
				...
				echo '<div id="fca-pc-setup-notice" class="notice notice-success is-dismissible" style="padding-bottom: 8px; padding-top: 8px;">';
					echo '<p>' . __( "Hi! You've been using Pixel Cat for a while now, so who better to ask for a review than you? Would you please mind leaving us one? It really helps us a lot!", $plugin_name ) . '</p>';
		...
		function fca_pc_admin_deactivation_survey( $hook ) {
			if ( $hook === 'plugins.php' ) {
				...
				<div id="fca-deactivate" style="position: fixed; left: 232px; top: 191px; border: 1px solid #979797; background-color: white; z-index: 9999; padding: 12px; max-width: 669px;">
					...
					<p><b><?php _e( 'If you\'re upgrading to Pixel Cat Premium and have questions or need help, click <a href=' . 'https://fatcatapps.com/article-categories/gen-getting-started/' . ' target="_blank">here</a></b>', 'facebook-conversion-pixel' ) ?>
					</p>
