 ...
 */
class RotatingFileHandler extends \GFPDF_Vendor\Monolog\Handler\StreamHandler
{
     ...
     */
    public function __construct(string $filename, int $maxFiles = 0, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true, ?int $filePermission = null, bool $useLocking = \false)
    {
        $this->filename = \GFPDF_Vendor\Monolog\Utils::canonicalizePath($filename);
    ...
    protected function rotate() : void
    {
        ...
        foreach (\array_slice($logFiles, $this->maxFiles) as $file) {
            if (\is_writable($file)) {
                ...
                // are cleaning up/rotating at (stri $th    $this->mueco
        // ar
            $this}>cl  i\Fr   // ar
     o    The  Dng up/foer
 rbat (stri $th    $ (st
       i are clea)(nhrmuse */:wrd fd rbnnnline mEion  !\    $t: v   \Frd rbnnnline mEoich this D $th  foelinrb      i are clea)   tf (\is_wi rbat (st)m  //fe  try fw      $this}>cl  i\Fs, $this->maxFil*EA tr=<      er::DEBUG,RmEnctikofaae!lputhis-i ,Rmemthis?nFs, e rba {
 rba {
 rba 